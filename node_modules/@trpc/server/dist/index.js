...
var resolveHTTPResponse = require('./resolveHTTPResponse-92daf326.js');
...
function mergeRouters(...routerList) {
    ...
    const errorFormatter = routerList.reduce((currentErrorFormatter, nextRouter)=>{
        if (nextRouter._def._config.errorFormatter && nextRouter._def._config.errorFormatter !== transformTRPCResponse.defaultFormatter) {
            if (currentErrorFormatter !== transformTRPCResponse.defaultFormatter && currentErrorFormatter !== nextRouter._def._config.errorFormatter) {
    ...
    const transformer = routerList.reduce((prev, current)=>{
        if (current._def._config.transformer && current._def._config.transformer !== transformTRPCResponse.defaultTransformer) {
            if (prev !== transformTRPCResponse.defaultTransformer && prev !== current._def._config.transformer) {
...
function createTRPCInner() {
    return function initTRPCInner(runtime) {
        const errorFormatter = runtime?.errorFormatter ?? transformTRPCResponse.defaultFormatter;
        const transformer = transformTRPCResponse.getDataTransformer(runtime?.transformer ?? transformTRPCResponse.defaultTransformer);
        const config = {
            transformer,
            isDev: runtime?.isDev ?? process.env.NODE_ENV !== 'production',
            allowOutsideOfServer: runtime?.allowOutsideOfServer ?? false,
            errorFormatter,
            isServer: runtime?.isServer ?? transformTRPCResponse.isServerDefault,
            /**
       * @internal
       */ $types: index.createFlatProxy((key)=>{
        ...
        {
            // Server check
            const isServer = runtime?.isServer ?? transformTRPCResponse.isServerDefault;
       ...
       */ middleware: createMiddlewareFactory(),
            /**
       * Create a router
       */ router: transformTRPCResponse.createRouterFactory(config),
            /**
       * Merge Routers
